...
un bus (`useMagiSocket.ts` ↔ `venim/core/bus.py`). Eso hace el mapa
...
_RUIDO = (
    "venim.gui", "venim.core", "venim.modules",       # rutas de import
...
def _raiz_repo(inicio: str | Path | None = None) -> Path | None:
    ...
    for c in (base, *base.parents):
        if (c / "venim").is_dir() and (c / "venim-gui").is_dir():
            return c
    return None
...
def _py(raiz: Path):
    return [p for p in (raiz / "venim").rglob("*.py")
