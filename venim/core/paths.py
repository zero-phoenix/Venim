...
Sustituye las 8 apariciones de "D:/PROYECTOS/Venim" que hacían que
...
_ENV_ROOT = "VENIM_ROOT"
_ENV_DATA = "VENIM_DATA_DIR"
_ENV_WORKSPACE = "VENIM_WORKSPACE"
...
def project_root() -> Path:
    ...
    - En desarrollo: dos niveles por encima de este fichero (venim/core/paths.py).
    - Sobrescribible con VENIM_ROOT para tests y despliegues raros.
    """
...
def data_dir() -> Path:
    ...
    if override:
        ...
        p = Path.home() / "Library" / "Application Support" / "Venim"
    else:
        base = os.environ.get("XDG_DATA_HOME") or os.path.expanduser("~/.local/share")
        p = Path(base) / "venim"
    if not override:
        _mudanza_desde_venicemagi(p)
    p.mkdir(parents=True, exist_ok=True)
    return p


#: Los nombres que tuvo esta carpeta antes de llamarse Venim.
_NOMBRES_ANTERIORES = ("VeniceMAGI", "MagiSystem")


def _mudanza_desde_venicemagi(nueva: Path) -> None:
    """Trae los datos del nombre anterior en vez de empezar de cero.

    POR QUÉ ESTO NO ES OPCIONAL
    ===========================
    Cambiar el nombre del programa cambió la carpeta de datos, y ahí no hay
    caché: están el historial de conversaciones, la carpeta de trabajo
    elegida, los artefactos generados y los informes. Arrancar Venim y
    encontrarse la casa vacía se parece muchísimo a haberlo perdido todo, y en
    la práctica lo es — nadie sabe que sus cosas siguen enteras dos carpetas
    más allá con el nombre viejo.

    Se RENOMBRA, no se copia: copiar dejaría dos historiales divergentes, y el
    día que alguien abra el ejecutable antiguo escribiría en el que ya nadie
    lee. Si el renombrado falla —la carpeta abierta en otro proceso, permisos—
    se deja como está y se avisa: perder los datos por intentar moverlos sería
    peor que el problema que se venía a resolver.
    """
    if nueva.exists():
        return
    for viejo_nombre in _NOMBRES_ANTERIORES:
        vieja = nueva.parent / viejo_nombre
        if not vieja.is_dir():
            continue
        try:
            vieja.rename(nueva)
            logging.getLogger(__name__).info(
                "[paths] tus datos de %s se han mudado a %s: el historial, la "
                "carpeta de trabajo y los artefactos siguen donde los dejaste.",
                viejo_nombre, nueva)
        except OSError as e:
            logging.getLogger(__name__).warning(
                "[paths] no se pudieron mudar los datos de %s a %s (%s). "
                "Siguen intactos en la carpeta anterior; muévelos a mano con "
                "el programa cerrado.", vieja, nueva, e)
        return
...
def workspace_dir() -> Path:
    ...
    =============================================================
    El 2026-09-05 le pedí a Venim que midiera un vídeo del repositorio.
    Resultado: `read_file` falló 8 de 8 veces (100%) y `run_command` 7 de 8
    (88%), todas contra rutas de la forma

        C:\\Users\\D\\AppData\\Local\\Venim\\workspace\\venim\\modules\\studio\\estilo.py
    ...
    Orden de precedencia, de más fuerte a más débil:
      ...
      1. `VENIM_WORKSPACE` en el entorno (para scripts y CI).
...
def db_path() -> Path:
    ...
    """
    return data_dir() / "venim_brain.db"
...
def escritorio() -> Path | None:
    ...
    dónde caer (workspace/entregas). `VENIM_DESKTOP` sobrescribe la ruta para
    pruebas y despliegues raros.
    """
    override = os.environ.get("VENIM_DESKTOP")
    if override:
        return Path(override).expanduser().resolve()
    if sys.platform == "win32":
        try:
