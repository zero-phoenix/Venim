# Compila VeniceMAGI en local Y COMPRUEBA QUE EL BINARIO ARRANCA.
#
#   powershell -ExecutionPolicy Bypass -File scripts\compilar_local.ps1
#
# POR QUE NO BASTA CON QUE PYINSTALLER TERMINE
# ============================================
# El 2026-09-06 PyInstaller termino sin un solo error y produjo un .exe de
# 143,9 MB que moria en el arranque, antes de ejecutar una linea del proyecto:
#
#     File "pyimod01_archive.py", line 134, in extract
#     zlib.error: Error -3 while decompressing data: incorrect header check
#
# `upx=True` en el .spec no significa "comprime": significa "comprime SI
# encuentras UPX en el PATH". Esta maquina lo tiene, el runner de GitHub no.
# La ventana abria en negro porque el servidor de la interfaz nunca llegaba a
# levantarse, y eso se parece mucho a "la interfaz no compila".
#
# Por eso este script no termina cuando termina PyInstaller: ARRANCA el .exe,
# espera a que su servidor conteste, y solo entonces dice que salio bien. Es la
# cuarta regla del proyecto —arrancar encuentra fallos que leer no encuentra—
# aplicada a la compilacion.
#
...
Set-Location (Join-Path $raiz "vmagi-gui")
cmd /c "npm run build 2>&1" | Select-Object -Last 4
Set-Location $raiz
if (-not (Test-Path (Join-Path $raiz "vmagi-gui\dist\index.html"))) {
  Write-Host "FALLO: no hay vmagi-gui/dist/index.html"; exit 1
}
$kb = (Get-ChildItem (Join-Path $raiz "vmagi-gui\dist\assets") -Filter *.js |
       Measure-Object Length -Sum).Sum / 1KB
Write-Host ("frontend OK - {0:N0} KB de JS" -f $kb)

Write-Host "=== 2/4 PYINSTALLER ==="
& $py -m PyInstaller --clean --noconfirm VeniceMAGI.spec 2>&1 | Select-Object -Last 6
...
$toc = Join-Path $raiz "build\VeniceMAGI\Analysis-00.toc"
...
if (-not $vivo) {
  ...
  if ((Test-Path $err) -and (Get-Item $err).Length -gt 0) {
    Write-Host "--- lo que dijo antes de morir ---"
