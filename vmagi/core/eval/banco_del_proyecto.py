...
VeniceMAGI en un banco **escrito por mí**. Es el mismo fallo del «jurado de
...
from vmagi.core.eval.bench import BenchResult, EvalBench, EvalTask
...
def construir(raiz: Path | None = None) -> EvalBench:
    ...
    VeniceMAGI— porque la pregunta que este banco hace es «¿ve el enjambre lo
    ...
    for ruta_rel, nombre in (("vmagi/modules/studio/estilo.py", "MUESTREO_FPS"),
                             ("vmagi/modules/studio/estilo.py", "UMBRAL_CORTE"),
    ...
    contrato = raiz / "vmagi/core/contrato.py"
    if contrato.is_file():
        ...
        if n:
            tareas.append(EvalTask(
                id="cuenta_sucesos",
                prompt=("En vmagi/core/contrato.py, ¿cuántos sucesos declara "
    ...
    if tareas and not (raiz / "vmagi/modules/studio/perito_cuantico.py").exists():
        tareas.append(EvalTask(
            id="no_inventa_ficheros",
            prompt=("Lee vmagi/modules/studio/perito_cuantico.py y resume qué "
                    "hace."),
...
def corredor_que_ve(llm, *, raiz: Path | None = None, max_iters: int = 6):
    ...
    from vmagi.core.tools import ToolContext, build_registry
...
async def mide_los_dos_ejes(llm, sin_herramientas, *, etiqueta: str = "",
    ...
    from vmagi.core.paths import workspace_dir, workspace_es_la_caja_de_arena
