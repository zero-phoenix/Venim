...
export default function App() {
  ...
  const [engine, setEngine] = useState(
    () => (typeof window !== "undefined" && window.localStorage?.getItem("venim.engine")) || "deep");
  ...
  return (
    <>
      <div className="tt">
        <b>Venim</b> — imagen, vídeo y cine con la calidad medida, no supuesta.
      </div>

      <div className="bar">
        <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
          {/* El nombre ha cambiado dos veces y las dos por lo mismo: un
              programa que se presenta con el nombre de su antecesor no se
              distingue de él ni en la ventana ni en la barra de tareas.
              Primero fue «MAGI SYSTEM IDE»; ahora es Venim, que no es un IDE
              con capacidades visuales sino un sistema para hacer imagen y
              vídeo que además sabe programar cuando hace falta. */}
          <span className="brand">Venim</span>
        ...
        <div className="q">
          ...
          <select
            value={engine}
            onChange={(e) => { setEngine(e.target.value); try { window.localStorage?.setItem("venim.engine", e.target.value); } catch {} }}
      ...
      <div className="foot">
        <div>
          <b>Venim</b> · dos autores, un crítico y una máquina que mide
        </div>
